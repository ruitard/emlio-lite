...
```
atext = ALPHA / DIGIT / "!" / "#" / "$" / "%" / "&" / "'" / "*" / "+" / "-" / "/" / "=" / "?" / "~" / "_" / "`" / "{" /
"|" / "}" / "~" atom = [CFWS] 1 * atext [CFWS] dot-atom-text = 1 * atext * ("." 1 * atext) dot-atom = [CFWS]
dot-atom-text [CFWS] quoted-pair = ("\SLASH" (VCHAR / WSP)) comment = "(" *([FWS] ctext / quoted-pair) [FWS] ")" CFWS =
(1 * ([FWS] comment) [FWS]) / FWS qtext = \%d33 / \%d35-\%d91 / \%d93-\%d126 ; printable ascii without backslash and
quote qcontent = qtext / quoted-pair quoted-string = [CFWS] DQUOTE * ([FWS] qcontent) [FWS] DQUOTE [CFWS] word = atom /
quoted-string phrase = 1 * word dtext = \%d33-\%d90 / \%d94-\%d126 ; printable ascii not including brackets and
backslash address = [phrase] angle-addr / addr-spec addr-spec = (dot-atom / quoted-string) "@" (dot-atom / dtext)
```
*/
...
public:
    ...
    **/
    ...
    Content type.
    **/
    struct EMLIO_EXPORT content_type_t {
        /**
    ...
    **/
    ...
    The method is used to parse message string line by line, as received over the protocols. For each line a CRLF is
    appended. If the line is CRLF only, then the parsing ends.

    If a message with mime parts is parsed but the message also contains a content, then the content is stored instead
    to be skipped.

    The method relies on the virtual `parse_header_line()` which should specify headers of interest to the class. The
    message class uses it for its own headers.
...
protected:
    ...
    **/
    ...
    @throw *          `bit7::encode(const string&)`, `bit8::encode(const string&)`, `base64::encode(const string&)`,
    `quoted_printable::encode(const string&)`.
    **/
    ...
    **/
    ...
    @throw * `bit7::decode(const string&)`, `bit8::decode(const string&)`, `base64::decode(const string&)`,
    `quoted_printable::decode(const string&)`.
    **/
    void parse_content();
    ...
    @throw *           `parse_header_name_value(const string&, string&, string&)`,
                       ...
                       `parse_content_transfer_encoding(const string&, content_transfer_encoding_t& encoding,
    map<string, string>&)`, `parse_content_disposition(const string&, content_disposition_t& disposition, map<string,
    string>&)`.
    **/
    ...
    **/
    void parse_header_name_value(const std::string &header_line, std::string &header_name,
    ...
    **/
    ...
    `mime_type_as_enum(const string&)`.
    **/
    ...
    @throw *                     `parse_header_value_attributes(const string&, string&, map<string, string>&)`.
    **/
    void parse_content_transfer_encoding(const std::string &transfer_encoding_hdr,
                                         content_transfer_encoding_t &encoding, attributes_t &attributes) const;
    ...
    **/
    ...
    The content is in the form as presented to the user, thus no limits such as line policy are applied here. For such
    purpose, the format method is used,
    **/
    std::string content_;
