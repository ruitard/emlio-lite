...
**/
template <typename Buf> struct String {
    Buf buffer;
    ...
    explicit String(const Buf &buffer_s, const std::string &charset_s = "ASCII") :
        buffer(buffer_s), charset{(std::transform(charset_s.begin(), charset_s.end(), charset_s.begin(),
...
template <typename Buf> bool operator==(const String<Buf> &lhs, const String<Buf> &rhs) {
...
template <typename Buf> bool operator>(const String<Buf> &lhs, const String<Buf> &rhs) { return operator<(rhs, lhs); }
...
template <typename Buf> bool operator==(const String<Buf> &lhs, const std::string &rhs) { return lhs.buffer == rhs; }

template <typename Buf> bool operator!=(const String<Buf> &lhs, const std::string &rhs) {
    return !operator==(lhs, rhs);
}
...
public:
    ...
    **/
    enum class line_len_policy_t : std::string::size_type {
        NONE = 2048,
        RECOMMENDED = 78,
        MANDATORY = 998,
        VERYLARGE = 16384
