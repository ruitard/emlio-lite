...
tuple<string, string, codec::header_codec_t> q_codec::check_decode(const string &text) const {
    ...
    for (auto ch = text.begin(); ch != text.end(); ch++) {
        ...
        if (*ch == codec::EQUAL_CHAR && ch + 1 != text.end() && *(ch + 1) == codec::QUESTION_MARK_CHAR && !is_encoded)
            is_encoded = true;
        else if (*ch == codec::QUESTION_MARK_CHAR && ch + 1 != text.end() && *(ch + 1) == codec::EQUAL_CHAR &&
                 question_mark_counter == QUESTION_MARKS_NO) {
            is_encoded = false;
...
bool q_codec::is_q_allowed(char ch) const { return (ch > SPACE_CHAR && ch <= TILDE_CHAR && ch != QUESTION_MARK_CHAR); }
