...
mime::mime() :
    version_("1.0"), line_policy_(codec::line_len_policy_t::RECOMMENDED),
    decoder_line_policy_(codec::line_len_policy_t::RECOMMENDED), strict_mode_(false), strict_codec_mode_(false),
    header_codec_(header_codec_t::UTF8), content_type_(media_type_t::NONE, ""),
        ...
        if (parsing_header_)
            ...
            // parsing the body
                ...
                if (line == BOUNDARY_DELIMITER + boundary_ && !boundary_.empty()) {
                    ...
                    // begin of another mime part means that the current part (if exists) is ended and parsed; another
                    // part is created
                    if (!parts_.empty())
...
mime::content_type_t mime::content_type() const { return content_type_; }

void mime::name(const string &mime_name) { name_ = mime_name; }

string mime::name() const { return name_; }

void mime::content_transfer_encoding(content_transfer_encoding_t encoding) { encoding_ = encoding; }

mime::content_transfer_encoding_t mime::content_transfer_encoding() const { return encoding_; }

void mime::content_disposition(content_disposition_t disposition) { disposition_ = disposition; }

mime::content_disposition_t mime::content_disposition() const { return disposition_; }

void mime::boundary(const string &bound) { boundary_ = bound; }
...
vector<mime> mime::parts() const { return parts_; }
...
codec::line_len_policy_t mime::line_policy() const { return line_policy_; }

codec::line_len_policy_t mime::decoder_line_policy() const { return decoder_line_policy_; }

void mime::strict_mode(bool mode) { strict_mode_ = mode; }

bool mime::strict_mode() const { return strict_mode_; }

void mime::strict_codec_mode(bool mode) { strict_codec_mode_ = mode; }

bool mime::strict_codec_mode() const { return strict_codec_mode_; }

void mime::header_codec(header_codec_t hdr_codec) { header_codec_ = hdr_codec; }

mime::header_codec_t mime::header_codec() const { return header_codec_; }
...
string mime::format_content_type() const {
    ...
    if (content_type_.type != media_type_t::NONE) {
        line += CONTENT_TYPE_HEADER + HEADER_SEPARATOR_STR + mime_type_as_str(content_type_.type) +
                CONTENT_SUBTYPE_SEPARATOR + content_type_.subtype;
        if (!content_type_.charset.empty())
            line += ATTRIBUTES_SEPARATOR_STR + content_type_t::ATTR_CHARSET + NAME_VALUE_SEPARATOR_STR +
                    content_type_.charset;
        if (!name_.empty()) {
            string mn = format_mime_name(name_);
            const string::size_type line_new_size = line.size() + ATTRIBUTES_SEPARATOR_STR.size() +
                                                    ATTRIBUTE_NAME.size() + NAME_VALUE_SEPARATOR_STR.size() +
        ...
        if (!boundary_.empty())
            line += ATTRIBUTES_SEPARATOR_STR + content_type_t::ATTR_BOUNDARY + NAME_VALUE_SEPARATOR_STR +
                    codec::QUOTE_CHAR + boundary_ + codec::QUOTE_CHAR;
...
string mime::format_transfer_encoding() const {
    ...
    switch (encoding_) {
    case content_transfer_encoding_t::BASE_64:
        line += CONTENT_TRANSFER_ENCODING_HEADER + HEADER_SEPARATOR_STR + CONTENT_TRANSFER_ENCODING_BASE64 +
                codec::END_OF_LINE;
        break;

    case content_transfer_encoding_t::QUOTED_PRINTABLE:
        line += CONTENT_TRANSFER_ENCODING_HEADER + HEADER_SEPARATOR_STR + CONTENT_TRANSFER_ENCODING_QUOTED_PRINTABLE +
                codec::END_OF_LINE;
        break;

    case content_transfer_encoding_t::BIT_8:
        line += CONTENT_TRANSFER_ENCODING_HEADER + HEADER_SEPARATOR_STR + CONTENT_TRANSFER_ENCODING_BIT8 +
                codec::END_OF_LINE;
        break;

    case content_transfer_encoding_t::BIT_7:
        line += CONTENT_TRANSFER_ENCODING_HEADER + HEADER_SEPARATOR_STR + CONTENT_TRANSFER_ENCODING_BIT7 +
                codec::END_OF_LINE;
        break;
...
string mime::format_content_disposition() const {
    ...
    case content_disposition_t::ATTACHMENT: {
        ...
        line += CONTENT_DISPOSITION_HEADER + HEADER_SEPARATOR_STR + CONTENT_DISPOSITION_ATTACHMENT +
                ATTRIBUTES_SEPARATOR_STR;
        const string::size_type line_new_size = CONTENT_DISPOSITION_HEADER.size() + HEADER_SEPARATOR_STR.size() +
                                                CONTENT_DISPOSITION_ATTACHMENT.size() + ATTRIBUTE_FILENAME.size() +
                                                sizeof(codec::EQUAL_CHAR) + sizeof(codec::QUOTE_CHAR) + name.size() +
                                                sizeof(codec::QUOTE_CHAR) + codec::END_OF_LINE.size();
        if (line_new_size >= string::size_type(line_policy_) - codec::END_OF_LINE.length())
            line += codec::END_OF_LINE + NEW_LINE_INDENT;
        line += ATTRIBUTE_FILENAME + NAME_VALUE_SEPARATOR_STR + codec::QUOTE_CHAR + name + codec::QUOTE_STR +
                codec::END_OF_LINE;
    ...
    case content_disposition_t::INLINE: {
        string name = format_mime_name(name_);
        line +=
            CONTENT_DISPOSITION_HEADER + HEADER_SEPARATOR_STR + CONTENT_DISPOSITION_INLINE + ATTRIBUTES_SEPARATOR_STR;
        const string::size_type line_new_size = CONTENT_DISPOSITION_HEADER.size() + HEADER_SEPARATOR_STR.size() +
                                                CONTENT_DISPOSITION_INLINE.size() + ATTRIBUTE_FILENAME.size() +
                                                NAME_VALUE_SEPARATOR_STR.size() + sizeof(codec::QUOTE_CHAR) +
                                                name.size() + sizeof(codec::QUOTE_CHAR) + codec::END_OF_LINE.size();
        if (line_new_size >= string::size_type(line_policy_) - codec::END_OF_LINE.length())
            line += codec::END_OF_LINE + NEW_LINE_INDENT;
        line += ATTRIBUTE_FILENAME + NAME_VALUE_SEPARATOR_STR + codec::QUOTE_CHAR + name + codec::QUOTE_CHAR +
                codec::END_OF_LINE;
...
/*
Some of the headers cannot be empty by RFC, but still they can occur. Thus, parser strict mode has to be introduced; in
case it's false, default values are set. The following headers are recognized by the parser:
- `Content-Type` cannot be empty by RFC 2045 section 5. In case it's empty, default value `text/plain` can be set.
- `Content-Transfer-Encoding` is not mandatory RFC 2045, section 6.1. In case it's empty, it should be seven bit by
default.
- `Content-Disposition` cannot be empty by RFC 2183, section 2. In case it's empty, consider it to be inline.

By RFC 2045 section 6.4, if an entity is of type Multipart, the content transfer encoding must be Seven Bit, Eight Bit
or Binary.
*/
...
void mime::parse_content_type(const string &content_type_hdr, media_type_t &media_type, string &media_subtype,
...
void mime::parse_content_transfer_encoding(const string &transfer_encoding_hdr, content_transfer_encoding_t &encoding,
                                           attributes_t &attributes) const {
    string header_value;
...
void mime::parse_content_disposition(const string &content_disp_hdr, content_disposition_t &disposition,
                                     attributes_t &attributes) const {
    string header_value;
...
string mime::mime_type_as_str(media_type_t media_type_val) const {
    ...
    case media_type_t::TEXT: mime_str = "text"; break;

    case media_type_t::IMAGE: mime_str = "image"; break;

    case media_type_t::AUDIO: mime_str = "audio"; break;

    case media_type_t::VIDEO: mime_str = "video"; break;

    case media_type_t::APPLICATION: mime_str = "application"; break;

    case media_type_t::MULTIPART: mime_str = "multipart"; break;

    case media_type_t::MESSAGE: mime_str = "message"; break;
    default: break;
    }
    return mime_str;
}
